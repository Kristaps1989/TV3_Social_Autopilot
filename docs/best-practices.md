...
| Franšīze iet visos kanālos, kur tā der | `franchise_channels` noteikumos (marķieris -> kanāli); grafiku zīmējam VIENU reizi, visiem kanāliem tas pats fails un tas pats laiks. Neaktīvs kanāls izkrīt pats, un kanāls, kura `formats` šo formātu nesatur, arī — tāpēc saišu franšīzes Instagramā nenonāk. X sarakstos nav apzināti: tur ieraksts ar saiti maksā $0.20 |
| Skaitli negriež, tekstu griež pa vārdu robežu | «Nedēļas skaitlis» ņēma `lines[0][:12]` un «50 miljoni dolāru» iznāca kā «50 miljoni d»: salauzts vārds, pazudusi mērvienība, nepatiess apgalvojums uz kartes. Fakts vai nu ietilpst, vai raksts ir nepareizais — karte rindu zīmē mazākā izmērā (`white-space:nowrap`, izmērs no platuma), un pārāk garu skaitli izlaiž. Teikumu (kontekstu) griezt drīkst, bet ar `best_practices.truncate_to` pa vārdu robežu |
| Redakcijas tagi pēc atbilstības, ne alfabēta | CMS tagus atdod alfabēta secībā, tāpēc «ASV» stāvēja pirms «Karš Ukrainā». `pagemeta.ranked_tags`: tags, kura vārdi ir virsrakstā/ievadā, iet pirmais; vispārīgie (`hashtag_generic`: valstis, sadaļas) — pēdējie. AI promptā tagi tajā pašā secībā ar norādi «tēma pirms personas, persona pirms valsts» |
| Threads: tēmas birka, ne # tekstā | `#Tags` tekstā Threads pārvērš birkā un atstāj kailu vārdu bez # — izskatās pēc kļūdas. `PlatformSpec.topic_tag`: birka cilvēka valodā («Karš Ukrainā») iet konteinera `topic_tag` parametrā un parādās galvenē «tv3.lv > Karš Ukrainā»; tekstā tās nav. Ja API birku noraida, ieraksts iet bez tās |
| Kanāla atslēgu pārsauc tikai ar migrāciju | Atslēga ir vienlaikus channels.yaml bloks, rules.yaml sarakstu vērtība un DB `posts.channel`. `config.CHANNEL_RENAMES` + `migrate_channel_keys` (YAML) un `db.migrate_channel_keys` (tabulas) pie starta; bez tā jaunā atslēga serverī ienāktu blakus vecajai kā OTRS kanāls ar dubultiem ierakstiem. Tā `threads_sport` kļuva par `threads_tv3lv` |
