...
| Threads | Threads API token for the account (`threads_basic`, `threads_content_publish`, `threads_manage_replies`, `threads_read_replies`) | Container→publish two-step; TEXT+link, IMAGE, VIDEO, CAROUSEL (≤20) and reply supported by the adapter; rate limit 250 posts/24 h |
